...
async def analyse_all_profiles(data_folder: str):
    ...
    # Так как PyTorch довольно массивна библиотека, а ранее в коде используются процессы, где каждый порожденный
    # процесс копируется с основного, то, для экономии памяти и времени, PyTorch загружается только после сбора данных
    ...
    for is_close in [False, True]:
        ...
        async for batch in generator:
            ...
            data_to_neuro = [(row[0], row[1:]) for row in batch]    # Убираем id из данных для нейронки
